...
local ts_page = 1
local SyntButton = CreateFrame("Button", "SyntButton", SpellBookFrame,
                               "UIPanelButtonTemplate")

SyntButton = Mixin(SyntButton, BackdropTemplateMixin)
SyntButton:SetText(
    "|cffff6188E|cfffc9867x|cffffd866p|cffa9dc76o|cff78dce8r|cffab9df2t|r")

SyntButton:RegisterForClicks("LeftButtonDown", "RightButtonDown")

SyntButton:SetWidth(60)
SyntButton:SetScript("OnClick", function(self) Tradeskill_Export() end)
SyntButton:SetPoint("RIGHT", SpellBookCloseButton, "LEFT", 6, 1);
syntFrame:RegisterEvent("ADDON_LOADED")
syntFrame:RegisterEvent("SKILL_LINES_CHANGED")
syntFrame:RegisterEvent("TRADE_SKILL_UPDATE")
syntFrame:RegisterEvent("TRADE_SKILL_CLOSE")
syntFrame:RegisterEvent("CRAFT_UPDATE")
syntFrame:RegisterEvent("CRAFT_CLOSE");
syntFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if event == "ADDON_LOADED" and arg1 == "SYNTradeSkillExport" then
        ...
        if export_data[player_name] == nil then
            export_data[player_name] = {}
        end
    end
    -------------------------------------------------------------------------------
    ...
    if event == "SKILL_LINES_CHANGED" then
        if export_data[player_name].spellbook_items == nil then
            export_data[player_name].spellbook_items = {}
        end
        local _, _, _, numSpells, _, _, _, _ = GetSpellTabInfo(ts_page)
        for i = 1, numSpells do
            _, spellbook_item_id = GetSpellBookItemInfo(i, syntVer)
            spellbook_item_name, _, _, _, _, _, _ = GetSpellInfo(
                                                        spellbook_item_id)
            export_data[player_name].spellbook_items[tostring(spellbook_item_id)] =
                spellbook_item_name
        end
    end
    -------------------------------------------------------------------------------
    ...
    if event == "TRADE_SKILL_UPDATE" then
        if export_data[player_name].professions == nil then
            export_data[player_name].professions = {}
        end
        local i2 = GetNumTradeSkills()
        for i = 1, i2 do
            ts_link = GetTradeSkillItemLink(i)
            if ts_link ~= nil then
                local _, _, _, _, Id = string.find(ts_link,
    ...
    if event == "CRAFT_UPDATE" then
        if export_data[player_name].crafts == nil then
            export_data[player_name].crafts = {}
        end
        local k2 = GetNumCrafts()
        for k = 1, k2 do
            craft_link = GetCraftItemLink(k)
            if craft_link ~= nil then
                local _, _, _, _, Id = string.find(craft_link,
                                                   "|?c?f?f?(%x*)|?H?([^:]*):?(%d+):?(%d*):?(%d*):?(%d*):?(%d*):?(%d*):?(%-?%d*):?(%-?%d*):?(%d*):?(%d*):?(%-?%d*)|?h?%[?([^%[%]]*)%]?|?h?|?r?")
...
function Tradeskill_Export()
    ...
    ser = syntDecoder .. ">>" .. export_data_json
    ...
    StaticPopupDialogs["EXPORT_TRADESKILL"] = {
        ...
        OnShow = function(self, data)
            self.editBox:SetText("syntradeskillexport" ..
                                     serialized_json_export_data)
...
function encode_json(val) return (encode(val)) end
